...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: localePath(locale, ROUTES.news),
      languages: Object.fromEntries(
        LOCALES.map((l) => [LOCALE_META[l].htmlLang, localePath(l, ROUTES.news)]),
      ),
    },
  };
}
...
export default async function NewsPage({
  ...
  const filter = CATEGORIES.includes(category as NewsCategory)
    ? (category as NewsCategory)
    : undefined;

  const [posts, featured] = await Promise.all([
    news.getPosts(filter),
    news.getFeaturedPost(),
  ]);

  // The featured post only leads the unfiltered view; inside a category the
  // newest post of that category should lead instead.
  const lead = filter ? null : featured;
  const rest = lead ? posts.filter((post) => post.slug !== lead.slug) : posts;

  const chips = CATEGORIES.map((key) => ({
    key,
    label: dict.news.categories[key],
    href:
      key === "all" ? at(ROUTES.news) : `${at(ROUTES.news)}?category=${key}`,
  }));

  return (
    <>
      <PageHero
        crumbs={[{ label: dict.news.title }]}
        title={dict.news.title}
      ...
      <Section>
        ...
        {lead && (
          <Reveal>
            <Link
              href={at(PATHS.newsPost(lead.slug))}
            ...
            >
              ...
              <div className="flex flex-col p-7 md:p-10">
                <div className="flex flex-wrap items-center gap-3">
                  <Chip tone="solid">{dict.news.featured}</Chip>
                  <Chip tone="muted">
                    {dict.news.categories[lead.category]}
                  </Chip>
