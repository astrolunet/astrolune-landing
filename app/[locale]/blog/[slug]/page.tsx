...
export async function generateMetadata({
  ...
  const path = PATHS.blogPost(slug);
  return {
...
export default async function BlogPostPage({
  ...
  const { prev, next } = await blog.getPostNeighbours(slug);

  // Missing translation falls back to the English original, stated as such —
    ...
    <>
      <section className="relative isolate overflow-hidden border-b border-line pt-[68px]">
        ...
        <div className="container-rail relative py-12 md:py-16">
          <Breadcrumbs
            items={[
              { label: dict.blog.title, href: at(ROUTES.blog) },
              { label: post.tag },
            ]}
          />

          <div className="mt-7 flex flex-wrap items-center gap-3">
            <Chip tone="muted">{dict.blog.categories[post.category]}</Chip>
          ...
          <p className="mt-6 label-mono text-ash-3">{dict.blog.byline}</p>

          {/* Cover at article width — an illustration for the piece, not a
              full-bleed banner. */}
      ...
      <Section>
        <article className="max-w-3xl">
          ...
          <div className="mt-14 flex flex-wrap items-center gap-3 border-t border-line pt-8">
            ...
            <ButtonGhost href={at(ROUTES.blog)} size="sm">
          ...
          <div className="mt-10">
            <PrevNext
              prev={
                prev
                  ? {
                      label: dict.blog.prevPost,
                      title: prev.title[active],
                      href: at(PATHS.blogPost(prev.slug)),
                    }
                  : null
              }
              next={
                next
                  ? {
                      ...
                      href: at(PATHS.blogPost(next.slug)),
                    }
                  : null
              }
            />
